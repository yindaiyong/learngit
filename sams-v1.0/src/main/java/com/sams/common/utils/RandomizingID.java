...
public class RandomizingID {
    ...
    /**
	 * 生成外部合同号   
	 * @Title: genOutContract   
	 * @author: wangchao 2020年06月11日 下午3:49:01
	 * @param: @param setupDate
	 * @param: @return      
	 * @return: String      
	 * @throws
	 */
    ...
    public synchronized String genOutContract(String setupDate) {
        if (currentSecondRandomNos.size() >= maxRequest) {
            try {
                Thread.currentThread();
                //计算当前秒距离下一秒还有多少毫秒
        ...
        String currentTime = sdf.format(c.getTime());
        //将年月日换成产品的成立日
        currentTime = setupDate+currentTime.substring(8, currentTime.length());
        
        if (!preGenTime.equals(currentTime)) {
            preGenTime = currentTime;
            currentSecondRandomNos.clear();
        }
        long random = getOutContractRandom();// 获取序列
        currentSecondRandomNos.add(random);
    ...
    /**
     * 获得外部合同号的序列
     * wangchao 20200612 下午2:20
     * @return Long
     */
    private Long getOutContractRandom() {
    	
    	interfaceColInfoMapper=(InterfaceColInfoMapper) SpringUtils.getBean(InterfaceColInfoMapper.class);
        Long sequenceId = interfaceColInfoMapper.selectOutContractSequence();
        return sequenceId;
    }
